...
Gaussians on priv at scoring time. Priv membership in the *target's*
training set is irrelevant for shadow training — we only need (image, label)
pairs, and both pub.pt and priv.pt provide those.

Recipe: SGD lr=0.1 momentum=0.9 nesterov, weight_decay=5e-4, cosine LR over
`epochs`, batch 256, augmentations = RandomCrop(32, padding=4) + HorizontalFlip,
then Normalize. Optional `label_smoothing` regularizes confidence; setting
LS=0.06 with epochs=60 produces shadows whose φ distribution closely matches
the target's, which is the second of the two shadow families used in the
final ensemble (the first is LS=0, epochs=100 — ordinary cross-entropy).

Saves to checkpoints/<ckpt_prefix>_NNNN.pt + <ckpt_prefix>_NNNN_in_idx.pt.
The in-idx tensor stores positions in the combined pool: [0, n_pub) → pub
samples, [n_pub, total) → priv samples.
"""
...
class IndexedSubset(Dataset):
    """Wraps a CombinedPool, yields (img, label) for the given indices, applying
    a fresh transform. Avoids mutating the underlying dataset."""
